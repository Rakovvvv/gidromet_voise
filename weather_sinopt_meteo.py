    ...
    if  curent_time == 6 and minutes >= 40:
        ...
        for part in email_message.walk():
            if part.get_content_type() == "text/plain":
                ...
                #проверяем записано ли уже в строку str_water письмо если не записано то записывает иначе пропускаем блок воды
                if str_water == '':
                    print('гидро 1')
                    if email_from.find(EMAIL_GIDROLOG) != -1:
                        print('гидро 1.1')
                        print('найдена почта гидрологов\n')
                        level_water         = ''
                        difference_water    = ''
                        difference          = ''
                        temperature_water   = ''
                        #находим пост гидрологов, затем все данные записываем в список
                        water_list = body[body.find('79256') + 5 : body.find('=')].split(' ')
                        #определяем уровень воды
                        level_water = water_list[2]
                        level_water = 'Уровень воды в Реке Сож города Гомель составляет ' + str(int(level_water[1:5])) + ' сантиметра. '
                        #достаём из списка разницу уровней
                        difference_water = water_list[3]
                        difference_water = difference_water[0:5]
                        #определяем умал ли уровень воды либо он поднялся
                        if difference_water[4] == '2':
                            difference = 'Убыло ' + str(int(difference_water[1:4]))  + ' сантиметров. '
                        elif difference_water[4] == '1':
                            difference = 'Вырос на ' + str(int(difference_water[1:4])) + ' сантиметров. ' 
                        
                        #определяем температуру воды
                        temperature_water = water_list[5]
                        temperature_water = temperature_water[0:5]
                        #temperature_water = 'Температура воды ' + str(float(temperature_water[1:3])) + ' градусов. '
                        if temperature_water[3:5] == '99':
                            temperature_water = 'Температура воды ' + str(int(temperature_water[1:3])) + ' градусов. '
                        elif temperature_water[3:5] == '//':
                            temperature_water = 'Температура воды ' + str(float(temperature_water[1:3])) + ' градусов. '
                        #формируем финальную строку где указываем уровень, сколько воды убыло или прибыло, а также её температуру
                        str_water =  str(level_water) + str(difference) + str(temperature_water)
                        print("Строка воды:", str_water)
                        #удлаяем письмо от гидрологов в 6 часов 30 минут
                        if curent_time == 6:
                            print('сообщения от гидрологов удалены')
                            mail.store(latest_email_uid, '+FLAGS', '\\Deleted')
...
output_file.write("По данным Гомель облгидро мет " + times + str_water + str_weather + str_meteo)
output_file.close()
